...
Genetic Algorithm to play breakout in a `<canvas>`
It's all based in the open source repository, so, the network files are implemented in here but the original informations are still here too.
